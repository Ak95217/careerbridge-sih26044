...
app.post('/api/ai/career-roadmap', async (req, res) => {
  try {
    ...
    const studentSkills = studentProfile?.skills?.map((s: any) => `${s.name} (${s.proficiency})`).join(', ') || 'No technical skills registered yet';
    const studentProjects = studentProfile?.projects?.map((p: any) => p.title).join(', ') || 'No projects listed yet';
...
- Key Skill Gaps to bridge: ${skillGaps.map((g: any) => typeof g === 'string' ? g : g.name).join(', ') || 'PostgreSQL, Docker, System Design, CI/CD'}
- Current Projects: ${studentProjects}
- Academic Level: Final Year B.Tech Computer Science
  ...
  "steps": [
    {
      ...
      if (parsed) {
        const roadmapResult = {
          ...
          targetRole,
          careerGoal,
          currentProficiencySummary: parsed.currentProficiencySummary || `Strong frontend foundations with React and TypeScript. Focus required on production backend scale, PostgreSQL optimization, and cloud deployments.`,
          targetTimelineWeeks: parsed.targetTimelineWeeks || 12,
          steps: (parsed.steps || []).map((st: any, idx: number) => ({
            stepNumber: st.stepNumber || idx + 1,
            phaseTitle: st.phaseTitle || `Phase ${idx + 1}`,
            skillsOrTopics: st.skillsOrTopics || ['Core Competency'],
            whyItMatters: st.whyItMatters || 'Essential for industry readiness.',
            suggestedPractice: st.suggestedPractice || 'Hands-on implementation.',
            suggestedProject: st.suggestedProject || 'Production-grade portfolio application.',
            expectedOutcome: st.expectedOutcome || 'Verified competency.',
            estimatedDuration: st.estimatedDuration || '2 Weeks',
            completed: false
          })),
    ...
    const fallbackRoadmap = {
      ...
      targetRole,
      careerGoal,
      currentProficiencySummary: `Your current profile displays high frontend readiness (React.js, TypeScript). Prioritize leveling up backend persistence (PostgreSQL), containerization (Docker), and distributed system design.`,
      targetTimelineWeeks: 12,
      steps: [
        {
          stepNumber: 1,
          phaseTitle: 'Strengthen Foundations & TypeScript Typing',
          skillsOrTopics: ['TypeScript Generics', 'Utility Types', 'Data Structures & Algorithms in TS'],
          whyItMatters: 'Top-tier tech companies evaluate deep typing and algorithmic efficiency in technical screening rounds.',
          suggestedPractice: 'Solve 20 medium-difficulty Graph and Tree problems on LeetCode using clean TypeScript.',
          suggestedProject: 'Type-safe event dispatcher and async state container library.',
          expectedOutcome: 'Confidently clear initial technical coding assessments with zero type assertions.',
          estimatedDuration: '2 Weeks',
          completed: false
        },
        {
          stepNumber: 2,
          phaseTitle: 'Backend Architecture & Secure REST/GraphQL APIs',
          skillsOrTopics: ['Node.js Event Loop', 'Express.js Middleware', 'JWT/OAuth2 Auth', 'Rate Limiting'],
          whyItMatters: 'Production backends require defensive security, request validation, and stateless session handling.',
          suggestedPractice: 'Implement an API gateway with token bucket rate limiting and RBAC middleware.',
          suggestedProject: 'Multi-tenant authentication and user identity management service.',
          expectedOutcome: 'Build hardened, production-grade microservice endpoints.',
          estimatedDuration: '2 Weeks',
          completed: false
        },
        {
          stepNumber: 3,
          phaseTitle: 'Relational Database Design & PostgreSQL Mastery',
          skillsOrTopics: ['SQL Indexes (B-Tree, GIN)', 'ACID Transactions', 'EXPLAIN ANALYZE', 'Prisma / Drizzle ORM'],
          whyItMatters: 'Database bottlenecks are the #1 cause of slow web applications at scale.',
          suggestedPractice: 'Design a normalized schema for an e-commerce order system with row-level locks on inventory.',
          suggestedProject: 'High-concurrency ticket booking reservation system with transaction isolation.',
          expectedOutcome: 'Optimize queries from 300ms to <10ms and prevent race conditions.',
          estimatedDuration: '2 Weeks',
          completed: false
        },
        {
          stepNumber: 4,
          phaseTitle: 'Build Full-Stack Production Artifact',
          skillsOrTopics: ['React.js + Node.js + PostgreSQL Integration', 'WebSockets / SSE', 'Tailwind CSS UI'],
          whyItMatters: 'Recruiters look for end-to-end full-stack capability with real-time bidirectional workflows.',
          suggestedPractice: 'Implement a real-time collaborative whiteboarding or chat feature using WebSockets.',
          suggestedProject: 'Real-time collaborative workspace with automated autosave and conflict resolution.',
          expectedOutcome: 'Deployable, standout portfolio project ready for live demonstration.',
          estimatedDuration: '3 Weeks',
          completed: false
        },
        {
          stepNumber: 5,
          phaseTitle: 'Containerization, CI/CD & Cloud Infrastructure',
          skillsOrTopics: ['Docker Multi-stage Builds', 'Docker Compose', 'GitHub Actions CI/CD', 'AWS EC2 / Cloud Run'],
          whyItMatters: 'Modern engineering teams expect engineers to own their build, test, and release pipelines.',
          suggestedPractice: 'Write GitHub Actions workflow that runs linter, tests, builds Docker image, and triggers automated deployment.',
          suggestedProject: 'Fully containerized multi-container app on AWS / Cloud Run with automated SSL.',
          expectedOutcome: 'Seamless zero-downtime deployment pipeline.',
          estimatedDuration: '2 Weeks',
          completed: false
        },
        {
          stepNumber: 6,
          phaseTitle: 'System Design & Placement Interview Readiness',
          skillsOrTopics: ['Horizontal Scaling', 'Caching with Redis', 'Load Balancing', 'Behavioral STAR Framework'],
          whyItMatters: 'Final round technical interviews focus heavily on trade-off analysis and architectural decisions.',
          suggestedPractice: 'Mock design sessions for URL Shortener, Twitter Feed, and Distributed Rate Limiter.',
          suggestedProject: 'Documented architectural whitepaper with high-availability load test benchmarks.',
          expectedOutcome: 'Ready to clear campus placements and Tier-1 product company interviews.',
