...
const getBenchmarkIdForTargetRole = (targetRole?: string) => {
  const role = (targetRole || '').toLowerCase();
  if (role.includes('data') || role.includes('ai') || role.includes('machine learning') || role.includes('scientist')) {
    return 'role-ai-engineer';
  }
  if (role.includes('cloud') || role.includes('devops') || role.includes('sre')) {
    return 'role-devops';
  }
...
export const SkillGapEngineView: React.FC<SkillGapEngineViewProps> = ({ onNavigateTab }) => {
  ...
  const [selectedTargetId, setSelectedTargetId] = useState<string>(() => {
    const targetRole = student?.targetRole || student?.careerGoal;
    const benchmarkId = getBenchmarkIdForTargetRole(targetRole);
    return opportunities.find(o => o.title.toLowerCase().includes((targetRole || '').toLowerCase()))?.id || benchmarkId;
  });
  ...
  useEffect(() => {
    const targetRole = student?.targetRole || student?.careerGoal;
    const benchmarkId = getBenchmarkIdForTargetRole(targetRole);
    const matchingOpportunity = opportunities.find(o => targetRole && o.title.toLowerCase().includes(targetRole.toLowerCase()));
    setSelectedTargetId(matchingOpportunity?.id || benchmarkId);
  }, [student?.id, student?.targetRole, student?.careerGoal, opportunities]);

  const handleTargetChange = (targetId: string) => {
    setSelectedTargetId(targetId);
    const selectedOpportunity = opportunities.find(o => o.id === targetId);
    const selectedBenchmark = CAREER_BENCHMARKS.find(benchmark => benchmark.id === targetId);
    const targetRole = selectedOpportunity?.title || selectedBenchmark?.title;
    if (targetRole) {
  ...
  if (liveOpp) {
    ...
    targetTitle = student?.targetRole || student?.careerGoal || bench.title;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        <div className="w-full md:w-80">
          ...
          <p className="text-[10px] text-slate-500 mt-1">Based on your profile target. Use the selector only to change target.</p>
        </div>
      </div>
            ...
            {targetRequiredSkills.map((req) => {
              const studentSkill = student?.skills?.find(
                s => {
                  ...
                  const requiredName = req.skillName.trim().toLowerCase();
                  return studentName === requiredName || studentName.includes(requiredName) || requiredName.includes(studentName);
                }
              );
                ...
                <div key={req.skillName} className="space-y-1.5">
                  <div className="flex items-center justify-between text-xs">
                    <div className="flex items-center gap-2">
                      ...
                      {studentSkill?.category === 'Soft Skills' && (
