      ...
      {filteredCertificates.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredCertificates.map((cert) => {
            ...
            const isNotSubmitted = cert.status === 'Not Submitted' || (!cert.status && !cert.documentFileName);
            ...
            return (
              ...
              >
                ...
                <div className={`absolute top-0 left-0 right-0 h-1 ${
                  isVerified ? 'bg-emerald-500' : isPending ? 'bg-amber-400' : isNotSubmitted ? 'bg-slate-300' : 'bg-rose-500'
                }`} />
      ...
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl max-w-lg w-full overflow-hidden max-h-[90vh] flex flex-col">
            ...
            {/* Modal Form Body */}
            <form onSubmit={handleSaveCertificate} className="p-4 sm:p-5 overflow-y-auto space-y-4 text-xs">
              {/* Certificate Title */}
              <div>
