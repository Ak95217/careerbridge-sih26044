...
export const StudentFoundationView: React.FC<StudentFoundationViewProps> = ({ onNavigateTab }) => {
  ...
  const scoredOpportunities = allOpps.map((opp) => {
  ...
  let avgMatchScore = 0;
  if (allOpps.length > 0 && studentSkills.length > 0) {
    const total = allOpps.reduce((acc, opp) => {
      const match = calculateSkillMatch(studentSkills, opp.requiredSkills || []);
      return acc + match.scorePercentage;
    }, 0);
    avgMatchScore = Math.round(total / allOpps.length);
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* Left: Current Mapped Skills & Quick Access (7 cols) */}
        ...
        {/* Right: Quick Module Actions (5 cols) */}
