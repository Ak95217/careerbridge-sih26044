...
import { calculateSkillMatch, CANDIDATE_MATCH_THRESHOLD } from '../../services/skillMatching';
...
export const CompanyCandidatesView: React.FC<CompanyCandidatesViewProps> = ({
  ...
  useEffect(() => {
    if (selectedOpportunityForMatching) {
      setActiveOpportunityId(selectedOpportunityForMatching.id);
    }
  }, [selectedOpportunityForMatching?.id]);
  ...
  const [minMatchFilter, setMinMatchFilter] = useState(String(CANDIDATE_MATCH_THRESHOLD));
  ...
  const filteredCandidates = useMemo(() => {
    return processedCandidates.filter(({ student, matchScore }) => {
      ...
      const minimumMatch = showBelowThreshold ? 0 : (Number(minMatchFilter) || CANDIDATE_MATCH_THRESHOLD);
  ...
  }, [processedCandidates, branchFilter, batchFilter, minCgpaFilter, minMatchFilter, showBelowThreshold, verifiedOnly, searchQuery]);

  const recommendedCandidates = useMemo(
    () => filteredCandidates.filter(({ matchScore }) => matchScore >= CANDIDATE_MATCH_THRESHOLD),
    [filteredCandidates]
  );
  ...
  return (
    <div className="space-y-6">
      ...
      {targetOpportunity && (
        <div className="px-4 py-3 bg-indigo-50/70 border border-indigo-200/80 rounded-2xl flex flex-wrap items-center justify-between gap-3">
          <div>
            <h3 className="text-sm font-bold text-slate-900">Candidates Matching This Opportunity</h3>
            <p className="text-xs text-slate-600 mt-0.5">Opportunity: <strong>{targetOpportunity.title}</strong></p>
          </div>
          <span className="text-xs font-bold text-indigo-800">Minimum Match: {CANDIDATE_MATCH_THRESHOLD}%</span>
        </div>
      )}
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {targetOpportunity && (
              <div className="w-40">
                ...
                >
                  <option value={String(CANDIDATE_MATCH_THRESHOLD)}>≥ {CANDIDATE_MATCH_THRESHOLD}% Qualified</option>
            ...
            {targetOpportunity && (
              <label className="flex items-center gap-2 cursor-pointer text-slate-700 font-medium">
                <input
                  ...
                  onChange={(e) => setShowBelowThreshold(e.target.checked)}
                  className="rounded text-indigo-600 focus:ring-indigo-500"
                />
                <span>Show candidates below 80%</span>
              </label>
            )}
          </div>

          <span className="text-slate-500">
            Showing <strong className="text-slate-900">{filteredCandidates.length}</strong> {targetOpportunity ? 'qualified candidates' : 'candidates'}
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between gap-3 px-1">
        <div>
          <h3 className="text-sm font-bold text-slate-900">
            {targetOpportunity ? `Candidates for: ${targetOpportunity.title}` : 'Talent Pool Explorer'}
          </h3>
          <p className="text-[11px] text-slate-500">
            Minimum Match: {CANDIDATE_MATCH_THRESHOLD}%
          </p>
        </div>
        <div className="text-[11px] text-slate-500">
          {recommendedCandidates.length} Recommended / {filteredCandidates.length} Visible
        </div>
      </div>
      ...
      {filteredCandidates.length === 0 ? (
        <Card>
          <CardContent className="p-12 text-center space-y-3">
            ...
            <h3 className="text-sm font-bold text-slate-900">
              {targetOpportunity ? 'No candidates currently meet the 80% match threshold.' : 'No Candidates Found'}
            </h3>
            <p className="text-xs text-slate-500 max-w-sm mx-auto">
              {targetOpportunity
                ? 'Try lowering the minimum match percentage or reviewing the opportunity requirements.'
                : 'Try loosening your filters or expanding your CGPA/Match score thresholds.'}
            </p>
            {targetOpportunity && !targetOpportunity.requiredSkills?.length ? (
              <p className="text-xs text-slate-500 max-w-sm mx-auto">
                Add required skills to this opportunity to generate candidate matches.
              </p>
            ) : null}
            <div className="pt-2">
              <Button
                ...
                onClick={() => {
                  ...
                  setBatchFilter('All');
                  setMinCgpaFilter('0');
                  setMinMatchFilter(String(CANDIDATE_MATCH_THRESHOLD));
