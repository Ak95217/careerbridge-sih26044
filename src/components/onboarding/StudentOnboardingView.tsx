...
import { StudentProfile, StudentSkill, StudentCertification, UserRole } from '../../types';
import { StorageService } from '../../services/storage';
import { getMissingStudentProfileFields, isStudentProfileComplete } from '../../services/profileCompletion';
...
export const StudentOnboardingView: React.FC<StudentOnboardingViewProps> = ({ onComplete }) => {
  ...
  const [graduationYear, setGraduationYear] = useState<number>(student?.graduationYear || 0);
  ...
  const [preferredWorkMode, setPreferredWorkMode] = useState<string>(student?.preferredWorkMode || '');
  const [careerDomain, setCareerDomain] = useState<string>(student?.careerDomain || '');
  ...
  const [customSkillCategory, setCustomSkillCategory] = useState<'Technical' | 'Soft Skills'>('Technical');
  ...
  const [certifications, setCertifications] = useState<StudentCertification[]>(student?.certifications || []);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const finalTargetRole = targetRole === 'OTHER' ? customTargetRole.trim() : targetRole.trim();
    const missingFields = getMissingStudentProfileFields({
      ...student,
      ...
      careerDomain: careerDomain.trim(),
    ...
    missingFields.forEach((field) => {
      if (field === 'Full Name' || field === 'Phone Number' || field === 'College / Institute' || field === 'Current Semester') {
        const errorKey = field === 'Full Name' ? 'fullName' : field === 'Phone Number' ? 'phone' : field === 'College / Institute' ? 'collegeName' : 'currentSemester';
        newErrors[errorKey] = `${field} is required`;
      }
    });

    if (Object.keys(newErrors).length > 0) {
      setErrors(newErrors);
      showToast('error', 'Incomplete Details', `Please complete: ${missingFields.join(', ')}.`);
      return;
    }
    ...
    if (!isStudentProfileComplete({ ...student, ...updatedProfile })) {
      setIsSubmitting(false);
      showToast('error', 'Incomplete Details', 'Please complete all required profile fields before continuing.');
      return;
    }
    ...
    setIsSubmitting(false);
    showToast('success', 'Profile Created Successfully', `Welcome to CareerBridge, ${fullName.trim()}!`);
    onComplete();
  };
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                ...
                <div>
                  ...
                  <select
                    id="student-current-semester-select"
                    value={currentSemester}
                    onChange={(e) => {
                      setCurrentSemester(e.target.value);
                      if (errors.currentSemester) setErrors({ ...errors, currentSemester: '' });
                    }}
                    className={`w-full px-3.5 py-2.5 bg-slate-900 border rounded-xl text-sm text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 ${errors.currentSemester ? 'border-rose-500 ring-1 ring-rose-500' : 'border-slate-700'}`}
                    required
                  >
                  ...
                  {errors.currentSemester && (
                    <p className="text-xs text-rose-400 mt-1 flex items-center gap-1">
                      <AlertCircle className="w-3 h-3" /> {errors.currentSemester}
                    </p>
                  )}
                </div>
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1.5">
                    CGPA / Percentage <span className="text-rose-400">*</span>
                  </label>
                  <input
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1.5">
                    Preferred Work Mode <span className="text-rose-400">*</span>
                  </label>
                  <select
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1.5">
                    Target Industry Domain <span className="text-rose-400">*</span>
                  </label>
                  <select
                    value={careerDomain}
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-300 mb-1.5">
                    Preferred Location(s) <span className="text-rose-400">*</span>
                  </label>
                  <input
                    type="text"
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {SOFT_SKILLS.map((skillName) => {
                    const selectedSkill = skills.find(s => s.name.toLowerCase() === skillName.toLowerCase());
                    return (
                      <button
                        key={skillName}
                        type="button"
                        onClick={() => handleToggleSoftSkill(skillName)}
                        className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all cursor-pointer ${
                          selectedSkill
                      ...
                      >
                        {selectedSkill && <CheckCircle2 className="w-3 h-3 inline mr-1 text-white" />}
                        {skillName}
                      </button>
                    );
                  })}
              ...
              <div className="bg-slate-900/80 border border-slate-700/80 rounded-xl p-3.5 space-y-3">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  ...
                  <select
                    ...
                    onChange={(e) => setCustomSkillCategory(e.target.value as 'Technical' | 'Soft Skills')}
                      ...
                      >
                        ...
                        {s.category === 'Soft Skills' && (
                          <select
                            ...
                            value={s.proficiency}
                            onChange={(e) => setSkills(skills.map(skill => skill.id === s.id ? { ...skill, proficiency: e.target.value as StudentSkill['proficiency'] } : skill))}
                            className="bg-indigo-950 text-[10px] text-indigo-200 border border-indigo-700 rounded"
                          >
                            <option value="Beginner">Beginner</option>
                            <option value="Intermediate">Intermediate</option>
                            <option value="Advanced">Advanced</option>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-slate-300">Key Projects ({projects.length}) <span className="text-slate-500 font-normal">Optional</span></span>
                  <button
                    type="button"
              ...
              <div className="space-y-2 pt-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-slate-300">Certifications ({certifications.length}) <span className="text-slate-500 font-normal">Optional</span></span>
                  <button
                    type="button"
