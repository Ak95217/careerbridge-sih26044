...
export const ResumeAnalyzerModal: React.FC<ResumeAnalyzerModalProps> = ({
  ...
  const [analysis, setAnalysis] = useState<AIResumeAnalysis | null>(
    initialAnalysis?.studentId === student.id && student.resumeFileName ? initialAnalysis : null
  );
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!content.trim()) throw new Error('No readable text was found in this resume.');
      setResumeText(content);
    } catch (error: any) {
      setResumeText('');
      setUploadError(error.message || 'Unable to read this resume file.');
    } finally {
      e.target.value = '';
    }
  };
  ...
  const handleRunAnalysis = async () => {
    if (!resumeText.trim()) {
      setAnalysisError('Upload your resume to run AI Resume Analysis.');
      return;
    }
    ...
    try {
      ...
      clearInterval(stepInterval);
      setAnalysisError(err.message || 'AI resume analysis failed. Please retry.');
    } finally {
        ...
        <div className="p-6 overflow-y-auto flex-1 bg-slate-50/40">
          ...
          {activeTab === 'upload' && (
            <div className="space-y-5">
              ...
              {!resumeText.trim() && !uploadError && (
                <p className="text-xs text-slate-500">Upload your resume to run AI Resume Analysis.</p>
              )}
              {analysisError && (
                <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-xs text-rose-700 flex items-center justify-between gap-3">
                  <span>{analysisError}</span>
                  <button type="button" onClick={handleRunAnalysis} className="font-bold underline">Retry</button>
                </div>
              )}
